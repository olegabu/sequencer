...
| `examples/counter/tests/websocket_transport_test.cpp` | `WebSocketTransport.BroadcastDeliversToConnectedClient`, `WebSocketTransport.MultipleMessagesArriveInOrder`, `WebSocketTransport.BroadcastToUnknownTopicIsANoOp` | A real Boost.Beast WebSocket client over a real socket, against a real `WebSocketTransport` server | The output side's WebSocket transport (`examples/counter`'s plug-in for `gateway/output`'s pluggable `OutputTransport`) actually delivers over the wire, not just compiles against Beast's API. |
