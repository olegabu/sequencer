...
`output_gateway_main.cpp` — still "the one place the
...
- **Output** (`output_gateway_main.cpp`, `--grpc_port`): wires
...
### All three flavors, one binary

`output_gateway_main.cpp` builds `counter_output_gateway`, which serves
any subset of the three transports — brpc (`--brpc_port`), real gRPC
(`--grpc_port`), WebSocket (`--websocket_port`) — from one process,
one journal tail and one `CounterOutputCodec` pass. Setting more than
one port is strictly less work than running separate processes, which
would tail the journal once each.

There were once three more binaries beside it, one per transport. They
were deleted: each was ~37 lines differing only in which transport it
constructed, so every chassis change had to be made four times over.
What they bought was a slimmer link — the WebSocket-only binary was
8.0 MB against the combined one's 24.1 MB, since the combined links
brpc *and* gRPC *and* Beast. That is a real saving for a deployment,
and an application still gets it the same way those mains did: link
only the transport library it needs (`gateway/output/` keeps them as
separate targets for exactly this) and construct that one transport.
It is not something an example needs three copies of.
...
brpc flavor was added to the counter example alongside it — the transport
